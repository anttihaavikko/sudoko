    ...
    SubShader{
        ...
        Pass{
			...
			fixed4 frag(v2f i) : SV_TARGET {
			    fixed4 noise = tex2D(_NoiseTex, i.uv * 0.5 + float4(_Time.x * 0.3, 0, 0, 0));
			    fixed4 second = tex2D(_NoiseTex, i.uv * 0.09 + float4(-_Time.x * 0.01, _Time.x * 0.01, 0, 0));
			    ...
			    fixed2 direction = normalize(float2((displace.r - 0.5) * 2, (displace.g - 0.5) * 2));
